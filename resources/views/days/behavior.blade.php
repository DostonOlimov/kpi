        ...
        <div class="page-header mb-4">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <i class="fa fa-calendar mr-1"></i>&nbsp;Odob-axloq qoidalari talablariga rioya etganligi
                </li>
            </ol>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tr>
                            ...
                            <th style="width: 50%"><i class="fa fa-calendar-check me-1"></i>Xodimning odob-axloq qoidalariga rioya etganligi</th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr data-user-id="{{ $user->id }}" class="user-row">
                                ...
                                <td>
                                    <div class="working-days-wrapper" data-user-id="{{ $user->id }}" data-user-name="{{ $user->first_name . ' '.$user->last_name }}">
                                        @php $user_score = $user->scores->where('kpi_id',9)->first() @endphp
                                        @if (!$user_score)
                                        ...
                                        @else
                                            <div class="working-days-display">
                                                    <span class="result-text badge bg-primary me-2">
                                                        <i class="fa fa-star me-1"></i>{{ round($user_score->score) }}
                                                    </span>
                                                <textarea class="form-control feedback-text"
                                                          placeholder="Izoh kiriting"
                                                          rows="2"
                                                          data-original-value2="0"
                                                          style="resize: vertical;" readonly>{{ $user_score->feedback }}</textarea>
                                                <button
                                                    class="btn btn-sm btn-primary edit-working-days-btn"
                                                    data-month="{{ $month_id }}"
                                                    data-year="{{ $year }}"
                                                    data-current-result="{{ $user_score->score }}"
                                                    data-current-feedback="{{ $user_score->feedback }}"
                                                    title="Tahrirlash (E)">
    ...
    <script>
        class WorkingDaysManager {
            ...
            async saveWorkingDays(userId, result, feedback, month, year) {
                const response = await fetch(`/days/create-behavior/${userId}`, {
                    method: 'POST',
                    headers: {
